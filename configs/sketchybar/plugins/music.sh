...
if pgrep -x "Spotify" > /dev/null 2>&1; then
  ...
  if [ "$SPOTIFY_PLAYING" = "playing" ]; then
    ...
    sketchybar --set $NAME drawing=on icon.color=$GREEN label="$ARTIST - $TRACK"
    exit 0
  fi
fi
...
if pgrep -x "Music" > /dev/null 2>&1; then
  ...
  if [ "$MUSIC_PLAYING" = "playing" ]; then
    ...
    sketchybar --set $NAME drawing=on icon.color=$RED label="$ARTIST - $TRACK"
    exit 0
  fi
fi

# Nothing playing - hide completely
